...
for i = 9:9
    ...
    for j = 1:200
        if j<28 
            PopOut(j) = 0;%春运期间湖北省日均输出客流量
        else
            PopOut(j) = 0;%平常期间湖北省日均输出客流量
        end
    end
    
    for idx = 1:length(T)-1
        S(idx+1) = S(idx) - round(r*B*S(idx)*I(idx)/N);
        E(idx+1) = E(idx) + round(r*B*S(idx)*I(idx)/N) - round(p*E(idx))...
            - round(PopOut(idx) * E(idx)/(S(idx)+R(idx)));
